...
\input{Tables/Results_SPD_N47}
\clearpage
\input{../manuscript/Figures/figure02_resultsN47}
\clearpage
...
\input{../manuscript/Figures/figureA2_othermethods}
...
\input{../manuscript/Figures/Alternative estimators}
\clearpage
...
\input{../manuscript/Figures/FigureA6_balancedpanel}
\clearpage
...
\input{../manuscript/Figures/Admin Results Uncapped}
\clearpage
...
\input{../manuscript/Figures/FigureA9_placebo}
\clearpage

\subsection{Randomization-Inference Placebo Test}
\label{sec:ri_appendix}

As a further robustness check, we implement a placebo/randomization-inference exercise: among the municipalities that eventually adopted \emph{Plan Cuadrante}, we randomly reassign each municipality's adoption date and re-estimate our specification under the sharp null of no treatment effect for any unit. The estimated direct effect on treated municipalities falls in the bottom first percentile of the resulting placebo distribution, confirming that our result is highly unlikely to arise from chance timing alone.

For this exercise, we restrict the placebo reassignment to the ever-treated municipalities, randomly reassigning each municipality's adoption date within the period for which we observe it, and re-estimate the direct effect under the sharp null $1{,}000$ times, yielding the placebo distribution in Online Appendix Figure~\ref{fig:ri_placebo}. We rely on the estimator of \citet{Chaisemartin} rather than that of \citet{Callaway} for this exercise: re-estimating the latter over a thousand permutations would require prohibitive computational power and running time. Because the placebo sample contains only municipalities that eventually adopted the program, it includes no genuinely untreated units, so it cannot be used to construct an analogous randomization-inference test for spillovers.
...
\input{../manuscript/Figures/figureA3_otherpreceptions}
\clearpage
...
\input{../manuscript/Figures/Robustness Earthquake}
\clearpage
...
\input{../manuscript/Figures/Spillovers with treatment intensity}
\clearpage

\input{../manuscript/Figures/figure03_spillovers}
\clearpage

\input{../manuscript/Figures/Spillovers with ENUSC sample}
\clearpage
